...
import { addMarkData, activeMark } from "../../SlateUtilityFunctions";
import { Transforms } from "slate";
import usePopup from "../../usePopup";
import { ReactEditor } from "slate-react";

const logo = {
  color: <MdFormatColorText size={24}/>,
  bgColor: <MdFormatColorFill  size={24}/>
};
const ColorPicker = ({ format, editor }) => {
  ...
  return (
    <div className="color-picker popup-wrapper-color" ref={colorPickerRef}>
      <button
        style={{
          color: showOptions ? "black" : activeMark(editor, format),
          opacity: "1"
        }}
        className={showOptions ? "picker-button-clicked" : "picker-button"}
